...
if __name__ == '__main__':
    ...
    print("start fit")
    clf = svm.SVC(kernel="poly",random_state=0, tol=1e-5)
    clf.fit(X_train, y_train)
    y_ = clf.predict(X_test)
    y_testS = np.reshape(y_test, (-1, 1))
    y_ = np.reshape(y_, (-1, 1))
    len = y_testS.shape[0]
    # totalY = np.vstack([y_core, y_core])
    # totalY = one_hot.fit_transform(totalY).toarray()
    # print(totalY)
    y_core = one_hot.fit_transform(y_testS).toarray()
    ...
    y_p = clf.decision_function(X_test)
    brier_score = 0
